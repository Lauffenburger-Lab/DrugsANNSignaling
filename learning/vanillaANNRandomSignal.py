...
for cell in cell_lines:
    for i in range(10):
        ...
        drugInput = pandas.read_csv('data/L1000_lvl3_allcells-conditions_drugs.tsv', sep='\t', low_memory=False, index_col=0)
        ...
        model1 = torch.load('../results/vanillaANNRandomSignal/models/'+pretrained_path)
        model1.eval()
        # if model_type==5:
        ...
        for e in range(e, maxIter):
            ...
            outString = 'Cell-line %s, random perm %s,Epoch=%s / %s'%(cell,i,e+1,maxIter)
        ...
        model.eval()
        Yhat = model(Xin)
        torch.save(model, '../results/vanillaANNRandomSignal/train/shuffled/'+cell+'_'+outPattern+'_'+str(i)+'.pt')
        ...
        plt.xlabel('Epoch')
        plt.yscale('log')
        plt.savefig('../results/vanillaANNRandomSignal/train/shuffled/fig1_'+cell+'_'+outPattern+'_'+str(i)+'.png',dpi=600)
        ...
        plt.savefig('../results/vanillaANNRandomSignal/train/shuffled/fig4_'+cell+'_'+outPattern+'_'+str(i)+'.png',dpi=600)

        plt.figure()
        rank = plotting.compareAllTFs(Yhat.detach().cpu(), Y.detach().cpu(), outNameGene)
        plt.savefig('../results/vanillaANNRandomSignal/train/shuffled/fig7_'+cell+'_'+outPattern+'_'+str(i)+'.png',dpi=600)
        plt.figure()
        rank = plotting.compareAllTFs(Yhat.detach().cpu().T, Y.detach().cpu().T, sampleName)
        plt.savefig('../results/vanillaANNRandomSignal/train/shuffled/fig8_'+cell+'_'+outPattern+'_'+str(i)+'.png',dpi=600)
        plotting.displayData(Y.detach().cpu(), sampleName, outNameGene)
        plt.savefig('../results/vanillaANNRandomSignal/train/shuffled/fig9_'+cell+'_'+outPattern+'_'+str(i)+'.png',dpi=600)
        ...
        plt.gca().set_xticks([0, 0.5, 1])
        plt.gca().set_yticks([0, 0.5, 1])
        plt.savefig('../results/vanillaANNRandomSignal/validation/shuffled/performance_'+cell+'_'+outPattern+'_'+str(i)+'.png',dpi=600)

        plt.figure()
        rank = plotting.compareAllTFs(Yhat.detach().cpu(), Y.detach().cpu(), outNameGene)
        plt.savefig('../results/vanillaANNRandomSignal/validation/shuffled/performancePerTF_'+cell+'_'+outPattern+'_'+str(i)+'.png',dpi=600)
        plt.figure()
        rank = plotting.compareAllTFs(Yhat.detach().cpu().T, Y.detach().cpu().T, outconds)
        plt.savefig('../results/vanillaANNRandomSignal/validation/shuffled/performancePerSample_'+cell+'_'+outPattern+'_'+str(i)+'.png',dpi=600)