...
class model(torch.nn.Module): ### CHANGED IT TO ADD THE DRUG LAYER: ADDED druginName and targetNames and targetList
    def __init__(self, networkList, nodeNames, targetNames, modeOfAction, inputAmplitude, projectionFactor, drugTargets,druginName,inName, outName, bionetParams, valType,drugSim=None,useMask=True,useDrugs=True,onlyECFP4Similarity=False):
        ...
        self.drugLayer = DrugstoSignal(drugTargets,len(druginName),len(targetNames),drugSim=drugSim,druginName=druginName,useMask=useMask,useDrugs=useDrugs,onlyECFP4Similarity=onlyECFP4Similarity)
...
### Drug layer ###
class DrugstoSignal(nn.Module):
    def __init__(self,mask, size1,size2,drugSim=None,druginName=None,momentum=0.6,drop=0.1,useMask=True,useDrugs=True,onlyECFP4Similarity=False):

        super().__init__()

        if (drugSim is None) and (druginName is None):
            raise Exception("One of the following arguments `drugSim` and `druginName` needs to have a value. Got None for both.")

        self.size_in = size1 ### drugs number
        self.size_out = size2 ### targets/signal size
        self.mask = mask.double()
        #self.batch_size = batch_size
        self.drugSim = drugSim
        self.useMask = useMask
        self.useDrugs = useDrugs
        self.onlyECFP4Similarity = onlyECFP4Similarity
        #self.trainableDrugs = trainableDrugs
        self.leak = 0.01

        # initialize weights and biases
        weights = torch.Tensor(size2,size1).double()
        nn.init.uniform_(weights, -0.3, 0.01) # weight init
        if useMask == True:
            weights = torch.mul(weights,self.mask).double()
        if useDrugs==True:
            if onlyECFP4Similarity==False:
                if drugSim is None:
                    ecfp4 = []
                    for drug in druginName:
                        if ((drug!='Ctrl_PBS') and (drug!='Ctrl_Cell')):
                            mol = Chem.MolFromSmiles(drug)
                            ecfp4.append(AllChem.GetMorganFingerprintAsBitVect(mol, 2, nBits=1024))
                        else:
                            ecfp4.append(numpy.zeros(1024))
                    self.ecfp4_fingerprints = torch.Tensor(numpy.array(ecfp4)).double()
                else:
                    self.ecfp4_fingerprints = torch.Tensor(numpy.zeros((len(druginName),1024))).double()
                if drugSim is None:
                    ecfp4_weights = torch.Tensor(self.ecfp4_fingerprints.shape[1], self.ecfp4_fingerprints.shape[1]).double()
                else:
                    ecfp4_weights = torch.Tensor(size1,size1).double()
                nn.init.uniform_(ecfp4_weights, 0., 1.)
                self.Wsim = nn.Parameter(ecfp4_weights, requires_grad=True)
                self.bn = nn.BatchNorm1d(num_features=size1, momentum=momentum, dtype=torch.double)
                self.dropout = nn.Dropout(drop)
            else:
                self.Wsim = drugSim
                self.bn = torch.Tensor(numpy.ones((mask.shape[0], mask.shape[1]))).double()
                self.dropout = nn.Dropout(drop)
        else:
            self.Wsim = torch.Tensor(numpy.ones((mask.shape[0],mask.shape[1]))).double()
            self.bn = torch.Tensor(numpy.ones((mask.shape[0],mask.shape[1]))).double()
            self.dropout = nn.Dropout(drop)

        self.A = nn.Parameter(weights, requires_grad = True)

    def forward(self, x):
        if self.useDrugs==True:
            if self.onlyECFP4Similarity == False:
                if self.drugSim is None:
                    W = torch.mm(torch.mm(self.ecfp4_fingerprints,self.Wsim),self.ecfp4_fingerprints.transpose(0, 1))
                else:
                    W = torch.mul(self.drugSim, self.Wsim)
            else:
                W = self.Wsim

        # W = torch.nn.functional.leaky_relu(W)
        if self.useMask == True:
            A = torch.mul(self.A,self.mask)
        if self.useDrugs == True:
            xhat = torch.mm(x,W)
            if self.onlyECFP4Similarity == False:
                xhat = self.bn(xhat)
                xhat = self.dropout(xhat)
            xhat=xhat.transpose(0, 1)
            if self.useMask == True:
                xhat = torch.mm(A, xhat)
            else:
                xhat = torch.mm(self.A, xhat)
        else:
            xhat = x.transpose(0, 1)
            if self.useMask == True:
                xhat = torch.mm(A, xhat)
            else:
                xhat = torch.mm(self.A, xhat)
