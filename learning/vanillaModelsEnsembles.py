...
parser.add_argument('--epochs', action='store', default=500) # 400 for gcnns  and 200 in new test set
parser.add_argument('--batchSize', action='store', default=25)
parser.add_argument('--ANNDense_hidden', action='store', default=2)
parser.add_argument('--no_of_ensembles', action='store', default=50)
...
no_of_ensembles = int(args.no_of_ensembles)
...
for c,cell in enumerate(cell_lines):
    ...
    Y_ALL_VAL = torch.zeros((no_of_ensembles, TFOutput_val.shape[0], TFOutput_val.shape[1])).to(device)
    Y_ALL_TRAIN = torch.zeros((no_of_ensembles, Y.shape[0], Y.shape[1])).to(device)
    
    if model_type in ['KNN','SVM']:
        ### For SVM and KNN use bootstrap with 100% to create ensembles
        for j in range(no_of_ensembles):
            all_data_train = numpy.concatenate((X.numpy(),Y.numpy()),axis=1)
            all_data_train = resample(all_data_train, n_samples = all_data_train.shape[0])
            if model_type == 'SVM':
                model = MultiOutputRegressor(SVR())
            else:
                model = KNN()
            model.fit(all_data_train[:,:-Y.shape[1]],all_data_train[:,-Y.shape[1]:])
            Yhat = model.predict(X_val)
            Yhat = torch.tensor(Yhat, dtype=torch.double).to(device)
            Y_ALL_VAL[j,:,:] = Yhat
            Y_ALL_TRAIN[j,:,:] = torch.tensor(model.predict(X.numpy()),dtype=torch.double).to(device)
        Yhat_mean = torch.mean(Y_ALL_VAL,0)
        Yhat_mean_train = torch.mean(Y_ALL_TRAIN, 0)
    else:
        if model_type == 'GCNN':
            Y = Y.float().to(device)
            Y_val = Y_val.float()
            for j in range(no_of_ensembles):
                ...
                for e in range(e, maxIter):
                    ...
                    outString = 'Cell-line %s, Model %s, Epoch=%s / %s' % (cell,j, e+1, maxIter)
                ...
                torch.save(model, '../results/vanilla_ensembles/'+model_type+'/models/' + cell +'model_no_'+str(j) +'.pt')
                Y_ALL_VAL[j,:,:] = Yhat
                Y_ALL_TRAIN[j,:,:] = model(X_atoms,X_bonds,X_edges)
            Yhat_mean = torch.mean(Y_ALL_VAL,0)
            Yhat_mean_train = torch.mean(Y_ALL_TRAIN, 0)
        else:
            X = X.to(device)
            ...
            for j in range(no_of_ensembles):
                ...
                Yhat = model(X_val)
                Y_ALL_VAL[j,:,:] = Yhat
                Y_ALL_TRAIN[j, :, :] = model(X)
                torch.save(model, '../results/vanilla_ensembles/' + model_type + '/models/' + cell+ '_model_no_'+ str(j) +'.pt')
            Yhat_mean = torch.mean(Y_ALL_VAL, 0)
            Yhat_mean_train = torch.mean(Y_ALL_TRAIN, 0)
    val_pear[c, :] = pearson_r(Y_val.detach(), Yhat_mean.detach()).detach().cpu().numpy()
    train_pear[c, :] = pearson_r(Y.detach().to(device), Yhat_mean_train.detach()).detach().cpu().numpy()
    if len(numpy.where(numpy.isnan(val_pear[c, :]))[0]):
        inds = numpy.where(numpy.isnan(val_pear[c, :]))[0]
        r = pearson_r(Y_val[:, inds].detach(),Yhat_mean[:, inds].detach() + 1e-8 * torch.randn(Yhat_mean.shape[0], Yhat_mean[:, inds].shape[1]).to(device))
        val_pear[c, inds] = r.detach().cpu().numpy()
    if len(numpy.where(numpy.isnan(train_pear[c, :]))[0]):
        inds = numpy.where(numpy.isnan(train_pear[c, :]))[0]
        r = pearson_r(Y[:, inds].detach().to(device),Yhat_mean_train[:, inds].detach() + 1e-8 * torch.randn(Yhat_mean_train.shape[0], Yhat_mean_train[:, inds].shape[1]).to(device))
        train_pear[c, inds] = r.detach().cpu().numpy()
    plt.figure()
    plt.scatter(Yhat_mean.detach().cpu().numpy(), Y_val.detach().cpu().numpy(), alpha=0.2)
    plotting.lineOfIdentity()
    plotting.addCorrelation(Yhat_mean.detach().cpu(), Y_val.detach().cpu())
    plt.xlabel('Fit')
    plt.ylabel('Data')
    plt.gca().axis('equal')
    plt.gca().set_xticks([0, 0.5, 1])
    plt.gca().set_yticks([0, 0.5, 1])
    plt.savefig('../results/vanilla_ensembles/'+model_type+'/test/performance_'+cell+'.png',dpi=600)

    plt.figure()
    rank = plotting.compareAllTFs(Yhat_mean.detach().cpu(), Y_val.detach().cpu(), outNameGene_val)
    plt.savefig('../results/vanilla_ensembles/'+model_type+'/test/performancePerTF_'+cell+'.png',dpi=600)
    plt.figure()
    rank = plotting.compareAllTFs(Yhat_mean.detach().cpu().T, Y_val.detach().cpu().T, outconds_val)
    plt.savefig('../results/vanilla_ensembles/'+model_type+'/test/performancePerSample_'+cell+'.png',dpi=600)
...
val_pear.to_csv('../results/vanilla_ensembles/'+model_type+'/val_Performance_perTF.csv')
...
train_pear.to_csv('../results/vanilla_ensembles/'+model_type+'/train_Performance_perTF.csv')
