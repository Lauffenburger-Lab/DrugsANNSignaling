...
performance <- read.xlsx('../results/regularizationTuneInteractionResults/hyperParamPerformanceResults.xlsx',
                         sheetIndex = 1)
...
performance_zero <- read.xlsx('../results/MultipleRuns/modeltype4.xlsx',sheetIndex=1)
performance_zero <- performance_zero %>% mutate(lambda=0) %>% group_by(lambda) %>% mutate(mean_corr=mean(corr)) %>%
  mutate(sd_corr=sd(corr)) %>% ungroup()
performance_inf <-  read.xlsx('../results/MultipleRuns/modeltype1.xlsx',sheetIndex=1)
...
shuffleX <- read.xlsx('../results/RandomSuffleX/test/randomShufflePerformance.xlsx',sheetIndex=1)
shuffleX <- shuffleX %>% select(-no) %>% mutate(lambda = 'shuffle X')
shuffleX <-shuffleX %>% group_by(lambda) %>% mutate(mean_corr=mean(corr)) %>%
  mutate(sd_corr=sd(corr)) %>% ungroup()
shuffleY <- read.xlsx('../results/RandomSuffleY/test/randomShufflePerformance.xlsx',sheetIndex=1)
...
files <- list.files('../results/MultipleRuns/test/')
...
files_reg <-  list.files('../results/regularizationTune/test/')
...
for (i in 1:length(all_files)){
  ...
  for (file in files_var){
    ...
    if (i==3) {
      file <- paste0('../results/regularizationTune/test/',file)
    }else{
      file <- paste0('../results/MultipleRuns/test/',file)
    }
...
print(p4)
png('../article_supplementary_info/performance_train_vs_validation.png',units = 'in',width = 12,height = 12,res = 600)
print(p4)
dev.off()
...
print(p5)
png('../article_supplementary_info/performance_train_vs_std.png',units = 'in',width = 12,height = 12,res = 600)
print(p5)
dev.off()
...
path <- '../results/regularizationNoiseSensitivity/'
...
#   saveRDS(df_null,paste0('../results/random_classification_performance_',colnames(cellInput)[j],'.rds'))
#   print(paste0('Finished cell ',colnames(cellInput)[j]))
# }
# #df_null <- data.frame(prec_inh_null,prec_mid_null,prec_act_null,f1_null,mcc_null)
# #saveRDS(df_null,'../results/random_classification_performance.rds')
# # Read saved simulations
# df_null <- data.frame()
# for (j in 1:ncol(cellInput)){
#   df_null <- rbind(df_null,
#                    readRDS(paste0('../results/random_classification_performance_',
